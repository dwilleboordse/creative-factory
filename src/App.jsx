...
export default function App() {
  ...
  const handleDocFiles = async (files) => {
    for (const f of files) {
      if (f.name.endsWith(".txt") || f.name.endsWith(".md") || f.name.endsWith(".csv") || f.type.startsWith("text/")) {
        ...
        {step === "setup" && (
          <div className="fi">
            ...
            <div style={{ background: C.card, borderRadius: 16, padding: 20, border: `1px solid ${C.border}`, marginBottom: 16 }}>
              ...
              <div style={{ fontSize: 12, color: C.textDim, marginBottom: 12 }}>Brand DNA, customer reviews, persona research, survey data, marketing materials. Upload .txt files or paste below.</div>
              ...
              <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
                <input ref={docRef} type="file" accept=".txt,.md,.csv,text/*" multiple style={{ display: "none" }} onChange={e => handleDocFiles(Array.from(e.target.files))} />
                <Btn v="sec" s="sm" onClick={() => docRef.current?.click()}>Upload .txt files</Btn>
              </div>
